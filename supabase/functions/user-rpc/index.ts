...
function makeCorsHeaders(req: Request): Record<string, string> {
  ...
  const allowed = ALLOWED_ORIGINS.includes("*") || ALLOWED_ORIGINS.includes(origin) || (isLocal && ALLOWED_ORIGINS.some((value) => value.startsWith("http://localhost")));
  return {
...
async function getUser(authHeader: string) {
  if (!SUPABASE_URL || !ANON_KEY || !SERVICE_ROLE_KEY) return { ok: false as const, status: 500, error: "server_not_configured" };
  ...
  if (error || !data?.user?.id) return { ok: false as const, status: 401, error: error?.message || "unauthorized" };
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "isUsernameAvailable") {
      ...
      if (!/^[a-zA-Z0-9_]{3,30}$/.test(username)) return json({ ok: true, data: false }, 200, corsHeaders);
    ...
    if (action === "handleReferralBonus") {
      ...
      if (!referralCode) return json({ ok: true, data: { ok: false, reason: "empty_code" } }, 200, corsHeaders);

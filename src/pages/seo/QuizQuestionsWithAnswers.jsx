import React from 'react';
import SeoLanding from './SeoLanding';

export default function QuizQuestionsWithAnswers() {
  return (
    <SeoLanding
      path="/quiz-questions-with-answers"
      title="Quiz Questions With Answers – Practice Online | Quiz Dangal"
      h1="Quiz Questions With Answers"
      description="Play quizzes on Quiz Dangal and review your results to learn from correct answers. Great for quick practice in GK, sports, movies, and more."
      keywords={['quiz questions with answers', 'quiz questions and answers', 'gk quiz', 'online quiz']}
      relatedLinks={[
        { href: '/quiz-questions/', label: 'Quiz Questions' },
        { href: '/category/gk/', label: 'GK Quiz' },
        { href: '/category/sports/', label: 'Sports Quiz' },
        { href: '/category/movies/', label: 'Movie Quiz' },
      ]}
      faqs={[
        {
          question: 'Where can I see answers?',
          answer: 'After finishing a quiz, Quiz Dangal shows results where the format allows — including correct answers and your score.',
        },
        {
          question: 'Is this suitable for exam practice?',
          answer: 'It can help with daily practice and speed. For full exam prep, use it alongside detailed study materials.',
        },
      ]}
    />
  );
}

import React from 'react';
import SeoLanding from './SeoLanding';

export default function OnlineQuiz() {
  return (
    <SeoLanding
      path="/online-quiz"
      title="Online Quiz – Play Daily Quizzes Online | Quiz Dangal"
      h1="Online Quiz"
      description="Play online quizzes on Quiz Dangal: opinion rounds, GK, sports, and movies. Quick rounds, leaderboards, and rewards-style coins."
      keywords={['online quiz', 'quiz game', 'daily quiz', 'quiz competition', 'quiz app']}
      relatedLinks={[
        { href: '/', label: 'Home' },
        { href: '/quiz-game/', label: 'Quiz Game' },
        { href: '/quiz-competition/', label: 'Quiz Competition' },
        { href: '/category/gk/', label: 'GK Quizzes' },
      ]}
      faqs={[
        {
          question: 'Is Quiz Dangal an online quiz app?',
          answer: 'Yes — it works in your browser like an app (PWA). You can play quizzes online without installing anything.',
        },
        {
          question: 'Do I need to create an account?',
          answer: 'You can browse public pages without login. For playing and tracking coins/streaks, sign in is recommended.',
        },
      ]}
    />
  );
}
